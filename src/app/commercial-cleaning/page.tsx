...
const commercialCleaningData = {
  ...
  whatsIncluded: {
    targetKeyword: "commercial cleaning melbourne",
    headline: <>What's Included in Our <span className="text-primary">Commercial Cleaning</span>?</>,
    seoParagraph: (
      <>
        Our commercial cleaning service helps businesses maintain a clean, hygienic, and professional environment for staff and visitors. We provide reliable cleaning solutions for offices, workplaces, and commercial spaces, ensuring common areas, workstations, and shared facilities are properly cleaned and maintained. Our trained cleaners follow a structured cleaning process to ensure consistent results across all areas of your workplace. From sanitising surfaces and cleaning bathrooms to vacuuming floors and maintaining common spaces, our professional commercial cleaners help create a cleaner and healthier workplace. The checklist below outlines the typical tasks included in our commercial cleaning services.
      </>
    ),
    items: [
      "Kitchen cleaning",
...
const GoogleReviewsCarousel = () => (
  <div className="w-full py-16 bg-gray-50 rounded-3xl flex flex-col items-center justify-center border border-gray-100 overflow-hidden relative">
    ...
    <div className="flex gap-4 opacity-50 absolute inset-0 items-center justify-center">
      {[1, 2, 3].map((i) => (
        <div key={i} className="w-64 h-32 bg-white rounded-xl shadow-sm border border-gray-100 p-4 shrink-0">
...
export default function CommercialCleaningPage() {
  const { hero, trustBar, whatsIncluded, features, pricingExplanation, finalCta, faqs } = commercialCleaningData;
  ...
  return (
    <main className="min-h-screen bg-background selection:bg-primary/20 selection:text-primary overflow-x-hidden font-sans">
            ...
            "@graph": [
              {
                ...
                "description": "Professional commercial cleaning service in Melbourne.",
                "address": { "@type": "PostalAddress", "addressLocality": "Melbourne", "addressRegion": "VIC", "addressCountry": "AU" },
                "areaServed": { "@type": "GeoCircle", "geoMidpoint": { "@type": "GeoCoordinates", "latitude": -37.8136, "longitude": 144.9631 }, "geoRadius": "50000" },
                "aggregateRating": { "@type": "AggregateRating", "ratingValue": "4.9", "bestRating": "5", "reviewCount": "250" }
              },
              { "@type": "Service", "serviceType": "Commercial Cleaning", "provider": { "@id": "https://crisp-cleaning-app-seven.vercel.app/#localbusiness" } },
      ...
      <div className="pt-24 lg:pt-32">
        <QuoteRequestPanel
          headline={<>{hero.headline.split("Melbourne")[0]}<span className="text-primary">Melbourne</span>{hero.headline.split("Melbourne")[1]}</>}
          subheadline={hero.subheadline}
          seoKeyword="commercial cleaning"
          contextPoints={["Top-rated commercial cleaning professionals in Melbourne.", "Flexible scheduling to fit your needs.", "100% Satisfaction Guarantee."]}
        />
      </div>
      ...
      <section className="bg-primary/5 border-y border-primary/10 py-6">
        <div className="container mx-auto px-6">
          <div className="flex flex-wrap justify-center lg:justify-between items-center gap-6 lg:gap-12">
            {trustBar.map((item, index) => { const Icon = item.icon; return (<div key={index} className="flex items-center gap-3"><div className="w-10 h-10 rounded-full bg-white shadow-sm flex items-center justify-center shrink-0 text-primary"><Icon className="w-5 h-5" /></div><span className="font-semibold text-foreground text-sm md:text-base">{item.text}</span></div>); })}
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-muted/30">
        <div className="container mx-auto px-6">
          <div className="text-center mb-10"><h2 className="text-3xl md:text-4xl font-display font-bold text-foreground tracking-tight">Loved by <span className="text-primary">Melbourne</span></h2></div>
      ...
      <section className="py-24 bg-background relative">
        <div className="container mx-auto px-6">
          <div className="max-w-4xl mx-auto text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-display font-bold mb-6 text-foreground tracking-tight">{whatsIncluded.headline}</h2>
            <div className="text-lg text-muted-foreground leading-relaxed text-left md:text-center p-6 bg-muted/20 rounded-2xl border border-border/50">{whatsIncluded.seoParagraph}</div>
          </div>
          <div className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 mb-12">
            {whatsIncluded.items.map((item, index) => (
              <div key={index} className="flex items-center gap-4 p-5 rounded-2xl bg-muted/30 border border-border hover:border-primary/30 hover:bg-primary/5 transition-all duration-300">
                <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center shrink-0"><CheckCircle2 className="w-5 h-5 text-primary" /></div>
                <span className="text-lg font-medium text-foreground">{item}</span>
              </div>
            ))}
          </div>
          <div className="text-center"><Button size="lg" className="rounded-full px-10 h-14 text-lg font-semibold shadow-lg shadow-primary/20">{whatsIncluded.cta}</Button></div>
        </div>
      </section>

      <section className="py-24 bg-muted/30">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16"><h2 className="text-3xl md:text-4xl font-display font-bold text-foreground tracking-tight">{features.headline}</h2></div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
            {features.items.map((feature, index) => { const Icon = feature.icon; return (<div key={index} className="bg-background p-8 rounded-3xl shadow-sm border border-border flex flex-col items-center text-center hover:shadow-md hover:-translate-y-1 transition-all duration-300"><div className="w-16 h-16 rounded-2xl bg-primary/10 flex items-center justify-center mb-6 text-primary shrink-0"><Icon className="w-8 h-8" /></div><h3 className="text-xl font-bold text-foreground">{feature.title}</h3></div>); })}
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-6">
          <div className="max-w-4xl mx-auto bg-primary text-primary-foreground rounded-[2rem] p-10 md:p-16 relative overflow-hidden shadow-2xl">
            ...
            <div className="relative z-10 text-center">
              <h2 className="text-3xl md:text-4xl font-display font-bold mb-6 tracking-tight">{pricingExplanation.headline}</h2>
              <p className="text-lg md:text-xl text-primary-foreground/90 max-w-2xl mx-auto mb-10 leading-relaxed font-medium">{pricingExplanation.description}</p>
              <Button size="xl" variant="secondary" className="rounded-full text-lg px-10 h-14 text-primary font-bold shadow-xl hover:shadow-2xl transition-all">{pricingExplanation.cta}</Button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-muted/30">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-display font-bold mb-4 text-foreground tracking-tight">See The Difference</h2>
            <p className="text-lg text-muted-foreground">Swipe to see our signature clean.</p>
          </div>
          <div className="max-w-5xl mx-auto"><BeforeAfterSlider /></div>
        </div>
      </section>
      ...
      <section className="py-24 bg-background relative z-10">
        <div className="container mx-auto px-6 space-y-24">
          <div>
            <div className="text-center mb-10"><h2 className="text-3xl md:text-4xl font-display font-bold text-foreground tracking-tight">Service <span className="text-primary">Areas</span></h2></div>
          ...
          <div className="w-full -mx-6 lg:mx-0"><FAQs data={faqs} /></div>
        </div>
      </section>
      ...
      <section className="py-24 md:py-32 relative overflow-hidden bg-foreground text-background">
        ...
        <div className="container mx-auto px-6 relative z-10 text-center">
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-display font-bold mb-10 tracking-tight text-white">{finalCta.headline}</h2>
          <Button size="xl" className="rounded-full text-lg px-12 h-16 shadow-2xl shadow-primary/20 bg-primary text-primary-foreground hover:bg-primary/90 font-bold">
            {finalCta.buttonText}
