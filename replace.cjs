...
const targetStart = '<div className="flex justify-between items-center py-3.5">\n        <span className="text-[12.5px] font-medium text-[#8d8378]">Service Type</span>';
const targetEnd = '          ${(pricingResult?.total || 0).toFixed(2)}\n        </span>\n      </div>';
...
const newBlock = `{currentStep === 7 ? (
        <div className="space-y-3 mt-1">
          ...
          {pricingResult?.discounts?.frequency && (
            <div className="flex justify-between items-center mb-1.5">
              <span className="text-[12.5px] font-semibold text-[#FB8C42]">Discount ({pricingResult?.discounts?.frequency?.name})</span>
...
const newCode = code.substring(0, startIndex) + newBlock.replace('ORIGINAL_BLOCK_PLACEHOLDER', originalBlock) + code.substring(endIndex);
